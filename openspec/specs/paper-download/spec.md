# Capability: paper-download

## Purpose

Exports papers from the library to any directory by copying PDFs, supporting selection by ID, search query, category, or all papers, with folder structure preservation and collision handling.
...
The system SHALL allow users to download papers matching a search query. The search SHALL use the same FTS5 search infrastructure as the existing `search` command.
...
The system SHALL allow users to download all papers in the library to a specified output directory.
...
The system SHALL preserve the category folder structure by default, creating `<output-dir>/<category>/<filename>.pdf`. A flat mode SHALL place all PDFs directly in the output directory root.
...
- **THEN** the system appends the paper's short ID to the second file's name to avoid overwriting

### Requirement: Collision handling

The system SHALL skip files that already exist in the output directory by default and report them. An overwrite flag SHALL force replacement of existing files.
...
The system SHALL gracefully handle papers whose source PDF file no longer exists on disk.
...
The system SHALL display a summary after the download completes, including the number of papers copied, skipped, and failed.

#### Scenario: Summary after download

- **WHEN** a download operation completes
- **THEN** the system displays: total requested, successfully copied, skipped (already exist), and failed (missing source)
